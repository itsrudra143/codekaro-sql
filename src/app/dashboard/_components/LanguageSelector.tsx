...
function LanguageSelector({ hasAccess = true }: { hasAccess?: boolean }) {
  ...
  // Define an array of freely accessible languages for non-pro users
  const freeLanguages = ["javascript", "java", "cpp"];
  ...
  const handleLanguageSelect = (langId: string) => {
    // Allow selection if user has access OR if the language is in the free list
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <motion.button
       ...
       ${!hasAccess && !freeLanguages.includes(LANGUAGE_CONFIG[language]?.id) ? "opacity-50 cursor-not-allowed" : ""}`}
        disabled={!hasAccess && !freeLanguages.includes(LANGUAGE_CONFIG[language]?.id)}
      >
        {/* Decoration */}
        <div
          ...
          >
            ...
            <div className="max-h-[280px] overflow-y-auto overflow-x-hidden">
              {Object.values(LANGUAGE_CONFIG).map((lang, index) => {
                // Language is locked if user doesn't have access AND it's not in the free list
